...
class TestLoggingIntegration(TestCase):
    ...
    def test_successful_execution_logging(self):
        ...
        try:
            ...
            log_messages = [call.args[0].getMessage() for call in self.mock_handler.handle.call_args_list]
            
            # Should have execution start message
            execution_msgs = [msg for msg in log_messages if "Executing:" in msg]
            self.assertTrue(len(execution_msgs) > 0, f"No execution messages in: {log_messages}")
            
            # Should have completion message
            completion_msgs = [msg for msg in log_messages if "completed in" in msg and "exit code" in msg]
            self.assertTrue(len(completion_msgs) > 0, f"No completion messages in: {log_messages}")
    ...
    def test_performance_logging_integration(self):
        ...
        try:
            ...
            perf_messages = [msg for msg in debug_messages if "execute_echo completed" in msg]
            self.assertTrue(len(perf_messages) > 0, f"No performance messages in: {debug_messages}")
    ...
    def test_log_level_hierarchy(self):
        ...
        logged_levels = [call.args[0].levelno for call in self.mock_handler.handle.call_args_list]
