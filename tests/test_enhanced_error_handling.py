...
class TestEnhancedErrorHandling(unittest.TestCase):
    ...
    def test_execution_error_enhanced_suggestions(self):
        ...
        error = ExecutionError(
            executable=executable, exit_code=exit_code, stderr=stderr, log_error=False
        )
    ...
    def test_error_context_preservation(self):
        ...
        def level2():
            try:
                ...
                raise SkogAIError(
                    f"Failed in level2: {e.message}", details=details
                ) from e
        ...
        def level1():
            try:
                ...
                raise SkogAIError(
                    f"Failed in level1: {e.message}", details=details
                ) from e
