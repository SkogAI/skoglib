...
class TestMainFunction(TestCase):
    ...
    def test_main_function_output(self, mock_exit, mock_stdout):
        ...
        self.assertIn("For programmatic usage: from skoglib import run_executable", output)
...
class TestPackageImports(TestCase):
    ...
    def test_public_api_completeness(self):
        ...
        expected_exports = {
            'run_executable', 'ExecutionResult',
            'SkogAIError', 'ExecutableNotFoundError', 'ExecutionError', 'ConfigurationError',
            'configure_logging', 'configure_from_env', 'get_logger', 'get_performance_logger',
            'SkogAIConfig', 'get_config', 'reset_config', 'find_executable', 
            'validate_executable', 'resolve_path', 'merge_configs',
        ...
        for export_name in skoglib.__all__:
            self.assertTrue(hasattr(skoglib, export_name), 
